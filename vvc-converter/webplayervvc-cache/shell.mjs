...
//const DEFAULT_BITSTREAM = "/demo/RA_Meridian_2560p59_BR1000000.mp4";
...
import VVCPlayer from "../cache/VVCPlayer.mjs";
import uPlot from "../cache/lib/uPlot.esm.min.js";
...
window.onload = async function () {
  ...
  player.onReady = (firstInit) => {
    updateUIButtons();
    // Удаляю автозапуск видео при загрузке страницы
...
function updateUIButtons() {
  const renditionsExist = player.renditions?.length;
  buttonDecR.hidden = buttonIncR.hidden = !renditionsExist;
  if (renditionsExist) {
  ...
  const statusActions = {
    stop: () => {
      ...
      Array.from(document.getElementsByClassName("disabledDuringPlayback")).forEach(e => e.disabled = false);
    },
    play: () => {
      buttonPlay.classList.replace("bi-play-fill", "bi-pause-fill");
      Array.from(document.getElementsByClassName("disabledDuringPlayback")).forEach(e => e.disabled = true);
    },
    pause: () => {
      buttonPlay.classList.replace("bi-pause-fill", "bi-play-fill");
      Array.from(document.getElementsByClassName("disabledDuringPlayback")).forEach(e => e.disabled = false);
  ...
  statusActions[player.playingStatus]?.();
